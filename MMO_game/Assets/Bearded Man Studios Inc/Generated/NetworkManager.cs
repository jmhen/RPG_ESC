...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] OnlineChatManagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is OnlineChatManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (OnlineChatManagerNetworkObject.Length > 0 && OnlineChatManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(OnlineChatManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateOnlineChatManager instead, its shorter and easier to type out ;)")]
		public OnlineChatManagerBehavior InstantiateOnlineChatManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(OnlineChatManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<OnlineChatManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<OnlineChatManagerBehavior>().networkObject = (OnlineChatManagerNetworkObject)obj;
		...
		public OnlineChatManagerBehavior InstantiateOnlineChatManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(OnlineChatManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<OnlineChatManagerBehavior>();
